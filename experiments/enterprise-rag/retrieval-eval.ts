...
const args = yargsParser(process.argv.slice(2), {
  ...
  default: {
    ...
    output: 'reports/enterprise-rag-retrieval-eval.jsonl',
    path: '/enterprise-rag/test',
    limit: 0,
    'result-limit': 10,
  },
});
...
const cases = (await readCases(casesPath))
  .filter((testCase) => testCase.positivePaths.length > 0)
  .slice(0, Number(args.limit) > 0 ? Number(args.limit) : undefined);
if (cases.length === 0) {
  throw new Error(`No answerable EnterpriseRAG-Bench eval cases found in ${casesPath}.`);
}
...
try {
  ...
  for (const arm of arms) {
    for (const testCase of cases) {
      ...
      rows.push(row);
      console.log(
        `[${arm}] ${testCase.question_id}: nDCG@10=${row.ndcgAt10.toFixed(3)} MRR@10=${row.mrrAt10.toFixed(3)}`,
      );
    }
  }
} finally {
...
function toMarkdown(rows: EvalRow[]): string {
  const lines = [
    '# EnterpriseRAG-Bench retrieval eval',
    '',
  ...
  lines.push('', '## Cases', '');
  lines.push('| question_id | type | arm | nDCG@10 | MRR@10 | positives | top hits |');
  lines.push('|---|---|---:|---:|---:|---|---|');
  for (const row of rows) {
    lines.push(
      `| ${escapeMd(row.question_id)} | ${escapeMd(row.questionType)} | ${row.arm} | ${row.ndcgAt10.toFixed(3)} | ${row.mrrAt10.toFixed(3)} | ${escapeMd(row.positivePaths.join('<br>'))} | ${escapeMd(row.retrievedPaths.slice(0, 5).join('<br>'))} |`,
    );
  }
  lines.push('');
  return lines.join('\n');
